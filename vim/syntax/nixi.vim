" Nixi syntax highlighting for Vim
...
syn keyword nixiBuiltin js eval querySelector addEventListener saveHTML renderHTML
...
" JavaScript blocks
syn region nixiJSBlock start=/js\s*"/ skip=/\\"/ end=/"/
syn region nixiJSBlock start=/js\s*'/ skip=/\\'/ end=/'/

" HTML tags
syn match nixiHTMLTag /\<\w\+\>/ contained
syn region nixiHTMLBlock start=/html\s*{/ end=/}/ contains=nixiHTMLTag,nixiString,nixiDelimiter
...
if version >= 508 || !exists('did_nixi_syn_inits')
  ...
  HiLink nixiJSBlock       Special
  HiLink nixiHTMLBlock     Special
  HiLink nixiHTMLTag       Identifier
